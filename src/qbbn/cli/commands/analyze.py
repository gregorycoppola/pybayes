...
def print_analysis(analysis, all_tokens, indent=0):
    """Recursively print analysis. Uses absolute indices into all_tokens."""
    prefix = "  " * indent
    
    if analysis.verb_index is not None:
        verb = all_tokens[analysis.verb_index]
...
def run(args):
    ...
    all_tokens = [t.corrected for t in corrected]
    
    text_analysis = TextAnalysis()
    
    for i, (start, end) in enumerate(segments):
        sent_tokens = all_tokens[start:end]
        print(f"\n--- Sentence {i+1}: \"{' '.join(sent_tokens)}\" ---")
        
        # Step 1: verb (pass offset for absolute indices)
        print("Finding main verb...")
        analysis = analyze_verb(sent_tokens, start, openai_client)
        
        # Step 2: arguments (recursive)
        print("Finding arguments...")
        analysis = analyze_args(sent_tokens, analysis, openai_client, recursive=True)
        
        print_analysis(analysis, all_tokens)
